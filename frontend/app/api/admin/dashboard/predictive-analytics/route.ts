...
export async function GET(request: NextRequest) {
  ...
  const apiUrl = `${backendUrl}/api/v1/admin/dashboard/predictive-analytics?timeRange=${timeRange}`;
  ...
  try {
    ...
    console.log(`Proxying predictive analytics request to: ${apiUrl} with token: ${token ? 'Bearer ' + token.substring(0, 10) + '...' : 'none'}`);
    ...
    console.error('Error proxying predictive analytics request to backend:', error);
