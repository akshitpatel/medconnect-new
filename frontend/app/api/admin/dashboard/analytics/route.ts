...
export async function GET(request: NextRequest) {
  ...
  // Use the correct URL path with hyphens as shown in the Rails routes
  const apiUrl = `${backendUrl}/api/v1/admin/dashboard/analytics?timeRange=${timeRange}`;
  ...
  try {
    ...
    console.log(`Proxying request to: ${apiUrl} with token: ${token ? 'Bearer ' + token.substring(0, 10) + '...' : 'none'}`);
    ...
    console.error('Error proxying analytics request to backend:', error);
